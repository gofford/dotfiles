...
5. **Share test resources when safe**
   - VPC networks, firewall rules (rarely change)
...
- [ ] No resource names repeat resource type (no `google_compute_network.main_network`)
...
```bash
# Step 1: Create secret in GCP Secret Manager (outside Terraform)
gcloud secrets create prod-db-password --data-file=- <<< "..."
...
│
├─ Secrets in state
│  └─ Use: GCP Secret Manager + write-only arguments (1.11+)
│
