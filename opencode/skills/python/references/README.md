...
**Important**: `/python` is for **general Python standards**, not Dagster-specific
patterns. For Dagster patterns, use `/dagster-best-practices`.

## Cross-Skill Usage

Users invoke `/python` when they need Python code quality guidance, regardless of whether
...
→ /python (check core-standards.md)
...
→ /dagster-best-practices (NOT python)
...
Users only invoke `/python` when they want Python standards guidance. The skill
